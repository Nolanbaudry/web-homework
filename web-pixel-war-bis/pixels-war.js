...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function refresh(user_id) {
        fetch(`${PREFIX}/deltas?id=${user_id}`, {credentials: "include"})
            .then((response) => response.json())
            .then((json) => {
                let delta = json.deltas
                console.log("delta")
                console.log(delta)
                delta.array.forEach(element => {
                    let div = document.getElementById(`${element[0]}-${element[1]}`)
                    div.style.backgroundColor=`rgb( ${element[2]}, ${element[3]},${element[4]})`
    ...
    function Add(nx,ny,data){
    console.log("add")
    let grid = document.getElementById("grid");
    grid.style.gridTemplateColumns=`repeat(${nx}, 20px)`
    grid.style.gridTemplateRows=`repeat(${ny}, 20px)`
    for (let i=0; i<nx; i++){
        for (let j=0; j<ny; j++){
            const div = document.createElement("div");
            div.id = `${i}-${j}`
            div.style.backgroundColor=`rgb( ${data[i][j][0]}, ${data[i][j][1]},${data[i][j][2]})`
